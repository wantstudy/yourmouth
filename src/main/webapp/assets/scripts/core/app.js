...
var App = function () {
    ...
    var swiftCookie=function(fn){
        if(fn)
            cookie=fn;
        else
            cookie=function(a,b){
            	if(typeof b!="undefined")
            	$.cookie(a,b,{expires:180});
            	else
            	return $.cookie(a);
            };
        //return cookie;
...
//      });
    ...
    //         e.preventDefault();
    //         if($(this).attr("action")=="removeform"){
    //             zkui.framework.pagerPost.closeLocker();
    //         }else if($(this).attr("action")=="hideform"){
    //             jQuery(this).closest(".portlet").hide("fast");              
    ...
    //         var fn = jQuery(this).attr("data-function");
    //         App.blockUI({target: el, iconOnly: true});
    //         if(fn){
    //             fn=fn.split(".");
    //             window[fn[0]][fn[1]](function(fun){window.setTimeout(function(){App.unblockUI(el);if(fun) fun();},1000)});  
    //         }
    //     });
  ...
  /*  var handleFancybox = function () {
    ...
    var handleTheme = function () {
        ...
        var setLayout = function () {
            ...
            cookie('sidebarPosOption', sidebarPosOption);
        ...
        var setColor = function (color) {
            ...
            $('#style_color').attr("href", "assets/css/themes/" + color_ + ".min.css");
            if (cookie) {                
        ...
        $('.toggler', panel).click(function () {
            $('.toggler').hide();
            $('.toggler-close').show();
            $('.theme-panel > .theme-options').addClass("easing").show();
        });
        ...
        if (cookie && cookie('headerOption')) {
            $('.layout-option', panel).val(cookie('layoutOption'));
            $('.sidebar-option', panel).val(cookie('sidebarOption'));
            $('.header-option', panel).val(cookie('headerOption'));
            $('.footer-option', panel).val(cookie('footerOption'));
            $('.sidebar-pos-option', panel).val(cookie('sidebarPosOption'));            
            setLayout();
        }
        if (cookie && cookie('style_color')) {
            var color=cookie('style_color');
            var colorbtn=$("div.theme-colors").find("ul").find("li");
            colorbtn.each(function(){
                $(this).removeClass("current"); 
                if($(this).hasClass("color-"+color))
                    $(this).addClass("current");                    
            });
            setColor(color);
        }
    ...
    return {
       ...
       // },
        ...
        alert: function(options) {
            ...
            $('#'+id).velocity({ opacity: [ 1, 0 ], translateY: [ 0, -20 ], translateZ: 0 } );
		...
		//$('#'+id).velocity({ opacity: [ 0, 1 ], translateY: -20, translateZ: 0 },function(){$('#' + id).remove();});
		$('#'+id).find("button[data-dismiss=alert]").click();
		$('#' + id).hide();
                $('#' + id).remove();
        ...
        toggleSidebarStatus:function(toggle){
        	if($("body").hasClass("temp")){
        		$("body").removeClass("page-sidebar-closed").removeClass("temp");
        	}
        	if(!$("body").hasClass("page-sidebar-closed")&&toggle){
				$("body").addClass("page-sidebar-closed temp");
			}
        },
        notice:function(msg,icon,position,time){
			time=time||2;
			var $toast=null;
			var title={"success":"成功","warning":"警告","error":"错误","info":"友情提示"};
			...
			//if(isremove)
			 toastr.remove();
			 $toast=toastr[icon](msg,title[icon]);
		},
